...
gf.stopGame = function () {
    var timeResult = $("#timer").text();
    $("#container").html("");
    $("#container").append("<div id='gameover'></div>");
    $("#gameover").append("<p>Congratulations!</p>").addClass("text");
    $("#gameover").append("<p id='time-result'></p>");
    
    $("#time-result").text("Your time: " + timeResult).addClass("text");
    $("#gameover").append("<div id='shared-fields'></div>");
    $("#shared-fields").append("<p><label for='name-player'>Your name:</label></p>");
    $("#shared-fields").append("<p><input type='text' id='name-player'></p>");
    $("#shared-fields").append("<p><a id='btn_save_results' href='#'>Save</a></p>")
    //$("#shared-fields").append("<input type='text'></p>");
    ...
    $("#gameover").append("<p id='shared-btn'></p>");
    $("#gameover").append("<input type='image' src='../../assets/share_button.png' id='share_button'>");
    $("#gameover").append("<div id='highscore'></div>");
    $("#highscore").append("<a id='btn_highscore' href='#'>View Highscore</a>")
    $("#highscore").append("<div id='scores-list'></div>")
    gf.callbacks.length = 0;

    window.fbAsyncInit = function () {
        FB.init({
            appId: '346107595517143', status: true, cookie: true,
            xfbml: true
        });
    };
    (function () {
    ...
    $(document).ready(function () {
        $('#share_button').click(function (e) {
            ...
            {
                method: 'feed',
                name: 'Ninja Survival game',
                link: 'http://ninjasurvival.antonyjekov.com',
                picture: 'http://ninjasurvival.antonyjekov.com/assets/logo.png',
                description: 'You survived ' + timeResult + ' in Ninja Survival.'
            });
    ...
    //  alert("Game is over! You can share your game experience on facebook by clicking the share button.");
    $(document).ready(function () {
        ...
        $("#highscore").on("click", loadHighScore);

        $("#btn_save_results").on("click", savePlayerScore);
    });

    function savePlayerScore() {
        var player = { name: $("#name-player").val(), score: timeResult };        
        var players = [];
        var i = 0;
        for (i = 0; i < localStorage.length; i++) {
            var key = i + 1;
            players.push(localStorage.getObject("player" + key));
        }
        localStorage.clear();
        ...
        for (i = 0; i <len ; i++) {
            var key = i + 1;            
            localStorage.setObject("player" + key, players[i]);
        }
    ...
    function loadHighScore() {
        ...
        var scoreHTML = "<table><tbody><tr><th>Player</th><th>Score</th></tr>";

        for (var i = 0; i < localStorage.length; i++) {
            var key = i + 1;
            var player = localStorage.getObject("player" + key);
            console.log(player);
            scoreHTML +=
            "<tr>" +
                  "<td>" +
